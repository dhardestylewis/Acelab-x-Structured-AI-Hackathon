...
`OPENROUTER_API_KEY` is optional. Without it, the deterministic extractor reports common schedule/spec mismatches. With it, two bounded reviewer calls add conservative comparisons for less regular layouts.
...
For local development, a `.env.local` file may define `OPENROUTER_API_KEY` and `OPENROUTER_MODEL`; it is ignored by Git. In graded runs, the sandbox-provided `OPENROUTER_API_KEY` takes precedence.
